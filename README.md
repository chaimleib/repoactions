...
For more control, `configure` currently allows you to set the installation prefix with the `--prefix=` option. You can also specify where the login code gets injected by using the RC variable (default is `~/.profile`):

```bash
RC="$HOME/.bashrc.d/repoactions.sh" ./configure --prefix=/opt
```
